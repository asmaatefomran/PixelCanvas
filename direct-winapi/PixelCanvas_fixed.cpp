...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    // Ensure fill states are properly initialized (default both to false)
    g_isFilled = false;
    g_polyFillEnabled = false;
    
    OutputDebugStringW(L"[DEBUG] Application started - Fill flags initialized to FALSE");
...
void CreateControls(HWND hwnd) {
    ...
    SendMessage(g_hFillCheckbox, BM_SETCHECK, BST_UNCHECKED, 0);
    OutputDebugStringW(L"[DEBUG] Checkbox initialized to UNCHECKED state");
...
void DrawEllipse(HDC hdc, int centerX, int centerY, int radiusX, int radiusY, COLORREF color, int thickness, bool filled)
{
    // Debug output to check fill value
    wchar_t debugMsg[256];
    _snwprintf(debugMsg, 256, L"DrawEllipse called with filled = %s", filled ? L"TRUE" : L"FALSE");
    OutputDebugStringW(debugMsg);
...
void UpdateControlsFromSelection() {
    ...
    // Get current checkbox state to avoid state changes during tool switching
    BOOL currentCheckState = (SendMessage(g_hFillCheckbox, BM_GETCHECK, 0, 0) == BST_CHECKED);
    
    // Determine what state should be reflected in the checkbox
    BOOL newCheckState;
    if (g_currentTool == ID_TOOL_POLYGON) {
        // For polygon tool, use the polygon-specific state
        newCheckState = g_polyFillEnabled ? BST_CHECKED : BST_UNCHECKED;
    } else {
        // For other tools, use the general state
        newCheckState = g_isFilled ? BST_CHECKED : BST_UNCHECKED;
    }
    
    // Only update if state actually changed to avoid flickering
    if (currentCheckState != newCheckState) {
        SendMessage(g_hFillCheckbox, BM_SETCHECK, newCheckState, 0);
    }
            ...
            switch (HIWORD(wParam)) {
                ...
                case BN_CLICKED:
                    ...
                    // Handle the fill checkbox
                        ...
                        // Update BOTH fill flags immediately regardless of current tool
        ...
        case WM_PAINT:
            {
                ...
                // Store current check state to restore after painting
                BOOL fillCheckState = SendMessage(g_hFillCheckbox, BM_GETCHECK, 0, 0);
                ...
                if (fillCheckState != SendMessage(g_hFillCheckbox, BM_GETCHECK, 0, 0)) {
                    SendMessage(g_hFillCheckbox, BM_SETCHECK, fillCheckState, 0);
                }
                ...
                if (x >= g_canvasRect.left && x <= g_canvasRect.right &&
                    ...
                    if (g_currentTool == ID_TOOL_POLYGON) {
                        ...
                        if (g_polygonPoints.size() < 4) {
                            ...
                            // Force redraw to show the point but ONLY redraw the canvas
                            InvalidateRect(hwnd, &g_canvasRect, FALSE);
                            ...
                            if (g_polygonPoints.size() == 4) {
                                ...
                                if (g_currentAlgorithm == ID_ALGO_CARDINAL || 
                                    ...
                                    // Force redraw but ONLY redraw the canvas
                                    InvalidateRect(hwnd, &g_canvasRect, FALSE);
                        ...
                        if (g_ellipseClickCount == 0) {
                            ...
                            try {
                                // Get the current checkbox state directly before drawing
                                BOOL isChecked = (SendMessage(g_hFillCheckbox, BM_GETCHECK, 0, 0) == BST_CHECKED);
                                g_isFilled = (isChecked == TRUE);
                                
                                // Debug output
                                wchar_t debugMsg[256];
                                _snwprintf(debugMsg, 256, L"[DEBUG] Drawing ellipse with fill = %s (checkbox state: %s)",
                                          g_isFilled ? L"TRUE" : L"FALSE",
                                          isChecked ? L"CHECKED" : L"UNCHECKED");
                                OutputDebugStringW(debugMsg);
                                
                                // Draw the ellipse with or without fill based on checkbox state
        ...
        case WM_MOUSEMOVE:
            {
                ...
                if (g_isDrawing) {
                    ...
                    // Force redraw but ONLY the canvas area, not the entire window
                    InvalidateRect(hwnd, &g_canvasRect, FALSE);
                }
                ...
                // Update status bar with coordinates (already done above)
        ...
        case WM_RBUTTONDOWN:
            {
                ...
                if (g_currentTool == ID_TOOL_POLYGON && g_polygonPoints.size() >= 3) {
                    ...
                    // Update g_polyFillEnabled based on checkbox state
                    g_polyFillEnabled = (isChecked == TRUE);
                    
                    // Debug output
                    wchar_t debugMsg[256];
                    _snwprintf(debugMsg, 256, L"[DEBUG] Polygon fill: checkbox state = %s, g_polyFillEnabled = %s", 
                              isChecked ? L"CHECKED" : L"UNCHECKED",
...
// Helper function to draw the 8 symmetric points of a circle
void DrawCirclePoints(HDC hdc, int centerX, int centerY, int x, int y, COLORREF color) {
...
void FillCircle(HDC hdc, int centerX, int centerY, int radius, COLORREF color) {
    ...
    for (int y = centerY - radius; y <= centerY + radius; y++) {
        ...
        DrawLineDDA(hdc, centerX - dx, y, centerX + dx, y, color);
    }
}
...
void UseGdiFallbackFill(HDC hdc, const std::vector<POINT>& points, COLORREF color) {
    ...
    // Use Windows GDI's Polygon function which is more reliable
    HBRUSH hBrush = CreateSolidBrush(color);
    HBRUSH hOldBrush = (HBRUSH)SelectObject(hdc, hBrush);
    HPEN hPen = CreatePen(PS_SOLID, 1, color);
    HPEN hOldPen = (HPEN)SelectObject(hdc, hPen);
    
    // Create a copy of the points for the GDI function
    POINT* polyPoints = new POINT[points.size()];
    for (size_t i = 0; i < points.size(); i++) {
        polyPoints[i] = points[i];
    }
    
    // Use the standard GDI Polygon function which is more reliable
    Polygon(hdc, polyPoints, (int)points.size());
